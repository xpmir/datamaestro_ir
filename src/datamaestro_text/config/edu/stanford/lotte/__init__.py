...
from datamaestro_text.data.ir.lotte import (
    LotteAssessments,
    LotteDocumentStore,
    LotteTopics,
)
from datamaestro_text.download.docstore import docstore_builder
...
class LotteDataset(Dataset):
    ...
    def config(self) -> Adhoc:
        data = LotteData.prepare()
        domain = self.DOMAIN
        split = self.SPLIT
        qtype = self.QTYPE
        return Adhoc.C(
            documents=LotteDocumentStore.C(
                path=data.datapath / f"store_{domain}_{split}",
            ),
            topics=LotteTopics.C(
                path=data.datapath / f"{domain}.{split}.{qtype}.queries.tsv",
            ),
            assessments=LotteAssessments.C(
                path=data.datapath / f"{domain}.{split}.{qtype}.qrels.jsonl",
            ),
        )
